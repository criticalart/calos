...
* **🎮 Gaming Friendly**: calOS ships with numerous gaming friendly configurations, all found within `games.conf` in the `/hypr` directory in your config folder. Features include a special workspace specifically for games, a custom gaming submap (with full waybar integration) that disables keybinds of your choosing + enables passthrough (for global vesktop/discord push to talk support) and Steam pre-installed (if chosen by the installer).
* **💻 Minimal System Apps**: Striving to be as bloat-free as possible, calOS primarily utilizes TUI applications/packages for system control. This includes `yazi` as your file browser, `cmus` for music, with `bluetui` and `impala` for bluetooth/wifi support.
...
For the uninitiated, your **Super** key is your windows key. Stop calling it that.
